% eq(in1,in2) : compares two objects for equality
...
if isa(in1,'cuda') && prod(size(in2)) == 1
    ...
    out.ref=cuda_cuda('equals_alpha',in1.ref,double(in2));
    out.fromDip = in1.fromDip;  
elseif prod(size(in1)) == 1 && isa(in2,'cuda')
    if isa(in1,'cuda')
        in1=double_force(in1);
    end
    out.ref=cuda_cuda('equals_alpha',in2.ref,double(in1));
...
elseif isa(in1,'cuda') && isa(in2,'cuda')
    ...
    out.ref=cuda_cuda('equals',in1.ref,in2.ref);
