...
%**************************************************************************
%
function out=subsasgn(in,index,val)
if ~isa(in,'cuda')
    if isa(val,'cuda')
        out=subsasgn(in,index,single_force(val));
        return;
    else
        error('Why is this function called for non-cuda variables?');
    end
end
switch index.type
    case '()'
        ...
        if isa(index.subs{1},'cuda') % referencing with a cuda image
            ...
            if in.fromDip == 0
                moffs=moffs-1;
                if length(moffs) <= 1  % single valued addressing
                    if (insize(1) > 1)
                        moffs=[moffs 0];
                        msize=[msize 1];
                    else
             ...
             if any((moffs + msize) > insize) || (isreal(in) && ~isreal(val))  % dataset needs expansion or change of datatype
                 ...
                 cuda_cuda('subsasgn_block',in.ref,tmp.ref,ns*0,insize,-1);  % no delete irgnored
                 in=tmp;
             end
            if isblock
