% xx_cuda2(in,varargin) :  mimics the dipimage xx function. See the dipimage function for documentation.
...
%**************************************************************************
%
function out=xx_cuda2(in,varargin)
    pos1=1;
    mysize=[];
    ...
    switch(location)
        case {'right','math','mright'}
            startVec=[0 -floor(dsize/2)];
            endVec=[0 floor((dsize+1)/2)];
        case {'corner','mcorner'}
            startVec=[0 0];
            endVec=[0 dsize];
        case {'true','mtrue'}
            startVec=[0 -dsize/2+0.5];
            endVec=[0 dsize/2+0.5];
        case {'left','mleft'}
            startVec=[0 -floor((dsize-1)/2)];
            endVec=[0 floor(dsize/2)+1];
        case {'freq','mfreq'}
            startVec=[0 -floor(dsize/2)/(2*floor(dsize/2))];
            endVec=[0 floor((dsize+1)/2)/(2*floor(dsize/2))];
