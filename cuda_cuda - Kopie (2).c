     ...
     if (cuda_array_dim[ref] == 1) {
        Dbg_printf3("creating 1D-plan with sizes : %d of type %s\n",cuda_array_size[ref][0],CUDA_TYPE_NAMES[cuda_array_type[ref]]);
         if (cuda_array_type[ref] == single)
             ...
             status=cufftPlan1d(&cuda_FFTplan[ref], cuda_array_size[ref],CUFFT_C2R,1); //cuda_array_origFTsize[ref]
        ...
        Dbg_printf4("creating 2D-plan with sizes : %dx%d of type %s\n",cuda_array_size[ref][0],cuda_array_size[ref][1],CUDA_TYPE_NAMES[cuda_array_type[ref]]);
         if (cuda_array_type[ref] == single)
             ...
             status=cufftPlan2d(&cuda_FFTplan[ref], cuda_array_size[ref][1], cuda_array_size[ref][1], CUFFT_C2R); // cuda_array_origFTsize[ref]
         else if (cuda_array_type[ref] == scomplex)
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if (strcmp(command,"put")==0) {  // -------------------------------------------
   ...
   Dbg_printf("cuda: fft3d\n");
  ...
  else if (strcmp(command,"rfft3d")==0) { // half complex fft
    ...
    if (mydim>3) mydim=3;
    oldsize=cuda_array_size[ref][mydim-1];
    cuda_array_size[ref][mydim-1] /= 2;cuda_array_size[ref][mydim-1] += 1;
    newarr=cudaAllocDetailed(cuda_array_dim[ref], cuda_array_size[ref], fftHalfSComplex);  // allocates the result
    cuda_array_size[ref][mydim-1]=oldsize;
    Dbg_printf3("rfft newarr has size %d %d\n",cuda_array_size[free_array][0],cuda_array_size[free_array][1]);

    status=cufftExecR2C(cuda_FFTplan[ref], getCudaRef(prhs[1]), (cufftComplex *) newarr);  // Out-of-place transform
