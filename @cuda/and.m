...
if isa(in1,'cuda') && prod(size(in2)) == 1
    ...
    out.ref=cuda_cuda('and_alpha',in1.ref,double(in2));
    out.fromDip = in1.fromDip;  
elseif prod(size(in1)) == 1 && isa(in2,'cuda')
    if isa(in1,'cuda')
        in1=double_force(in1);
    end
    out.ref=cuda_cuda('alpha_and',in2.ref,double(in1));
    out.fromDip = in2.fromDip;  
elseif isa(in1,'cuda') && isa(in2,'cuda')
%     if ((~in1.fromDip || ~in2.fromDip) && ~compatible(size(in1),size(in2)))
%         error('cuda:and of Matlab array type: Matrix dimensions must agree.')
%     end
    ...
    out.ref=cuda_cuda('and',in2.ref,in1.ref);
