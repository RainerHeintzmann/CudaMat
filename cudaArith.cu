...
{                                                               \
  ...
  if (cudaGetLastError() != cudaSuccess)                        \
      return cudaGetErrorString(cudaGetLastError());            \
                                                                \
  cudaMemcpyFromSymbol(resp, "cuda_resultVal", sizeof(* resp), 0, cudaMemcpyDeviceToHost);\
  if (cudaGetLastError() != cudaSuccess)                        \
      return cudaGetErrorString(cudaGetLastError());            \
  return 0;                                                     \
}
...
{                                                               \
  ...
  if (cudaGetLastError() != cudaSuccess)                        \
      return cudaGetErrorString(cudaGetLastError());            \
                                                                \
  cudaMemcpyFromSymbol(pM, "cuda_resultInt", sizeof(int), 0, cudaMemcpyDeviceToHost);\
                                                                \
    if (cudaGetLastError() == cudaSuccess)                      \
        return 0;                                               \
    else                                                        \
...
extern "C" int CUDAmaxSize() {
    ...
    return prop.maxGridSize[0]*prop.maxThreadsDim[0];   // 65535  = max GridSize = max nBlocks?
}
