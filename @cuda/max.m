...
if isa(in1,'cuda') 
    if in1.fromDip
        if (nargin < 3)  % max over all pixels
            ...
            elseif projdir(p) == 2
                ...
                if nargout > 1
                    ...
                    [valref,posref]=cuda_cuda('part_max',inref,mask,projdir(p));
                    val.ref=valref;pos.ref=posref;
                else
                    val.ref=cuda_cuda('part_max',inref,mask,projdir(p));
                end
                if p > 1
        ...
        if length(projdir) > 1
            error('cuda/max: Matlab type arguments can only be projected over a maximum of one dimension at a time.');
        end
        si=size(in1);
        if prod(si) ~= max(si)
            val=cuda();
            val.ref=cuda_cuda('part_max',in1.ref,[],projdir);
            val.fromDip = 0;
            % error('cuda/sum: Patial sum of multidemensional arrays in Matlab style not yet supported.');
        else
            val=cuda_cuda('max',in1.ref);
        end
    end
else
    error('cuda/max: Unknown datatype');
end
