...
static float * pOne=0, * pZero=0, * pReturnVal;
static int NumZero=-1,NumOne=-1,NumReturnVal=-1;
...
#endif
    printf("----------------------------------------------------------------------------------\n");
    printf("Summary: Memory used %9d, Heap %9d, Total on card %9d, Allocated %9d \n",sumMem,sumHeap,sumMem+sumHeap,SumAllocated);
...
/* void ReduceToHalfComplex(int pos) {
 ...
 float * cudaReturnVal(const mxArray * arg) {   // sets the result value to zero and returns the pointer
    cudaError_t err;    
    err=cudaMemset(pReturnVal,0,2*sizeof(float));
...
cufftHandle CreateFFTPlan(int ref, int PlanType) {    // returns the correct plan to use, or creates a new plan
    ...
    // printf("FFT Error codes: %d, %d, %d, %d, %d ,%d ,%d, %d, %d, %d\n",CUFFT_SUCCESS,CUFFT_INVALID_PLAN,CUFFT_ALLOC_FAILED,CUFFT_INVALID_TYPE,CUFFT_INVALID_VALUE,CUFFT_INTERNAL_ERROR, CUFFT_EXEC_FAILED, CUFFT_SETUP_FAILED, 0, CUFFT_INVALID_SIZE);
     ...
     else if (numdims  == 2 || (cuda_array_dim[ref] > 2 && cuda_array_size[ref][2] == 1)) {
...
CheckMemoryConsistency();
  ...
  if (strcmp(command,"put")==0) {  // -------------------------------------------
   ...
   p_cuda_data1=0; // jsut to eliminate a warning
    ...
    ReleaseAllFFTPlans();
    cudaDelete(NumReturnVal);  // release the fixed numbers
    cudaDelete(NumZero);  // release the fixed numbers
    cudaDelete(NumOne);
    ClearHeap();
    cublasShutdown();
#ifndef NOCULA
    ...
    if (nlhs > 0)
        plhs[0] =  mxCreateDoubleScalar((double) cudaGetVal(pReturnVal) == 0);
    ...
    if (nlhs > 0)
        plhs[0] =  mxCreateDoubleScalar((double) cudaGetVal(pReturnVal));
   ...
   Dbg_printf("cuda: rfft3d\n");
    ...
    {   status=cufftExecC2R(myPlan, srcstart, dststart);  // Out-of-place transform  (cufftComplex *) 
        if (status != CUFFT_SUCCESS) {printf("Error: %s",ERROR_NAMES[status]);mexErrMsgTxt("cuda: Error rifft3d failed\n");return;}
    }
  ...
  // cuda_array_FTscale[free_array]
   ...
   Dbg_printf("cuda: sprod\n");
    ...
    cublasSaxpy (getTotalSizeFromRef(prhs[1]), (float) alpha, pOne, 0, mynew, 1);  // y = alpha * x + y
