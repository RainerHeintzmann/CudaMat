% gt(in1,in2) : greater than function
...
if isa(in1,'cuda') && prod(size(in2)) == 1
    ...
    out.ref=cuda_cuda('larger_alpha',in1.ref,double(in2));
    out.fromDip = in1.fromDip;  
elseif prod(size(in1)) == 1 && isa(in2,'cuda')
    if isa(in1,'cuda')
        in1=double_force(in1);
    end
    out.ref=cuda_cuda('alpha_larger',in2.ref,double(in1));
...
elseif isa(in1,'cuda') && isa(in2,'cuda')
    ...
    out.ref=cuda_cuda('larger',in2.ref,in1.ref);
